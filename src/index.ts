...
async function main() {
  ...
  try {
    ...
    // ========================================================================
    console.log(
      "Step 5: Creating tracking survey with initial answers (Wave 1)...",
    );
    ...
    const codeFrameJob = await client.createCodeFrame(surveyId, questionId, {
      additionalInstruction:
        "Create categories for: sentiment (positive/negative/neutral), feature requests, bug reports, performance issues, and UX feedback",
      tierCount: 2,
    });
    ...
    console.log(
      `✓ Code frame created with ${countTotalCodes(codeFrame)} categories`,
    );
    ...
    const evalResponse = await client.createEvaluation(surveyId, questionId, {
      ...
      additionalInstructionEvaluation:
        "Assign multiple codes if applicable. Focus on actionable insights.",
    });
    ...
    const wave1Results = await client.getEvaluation(
      surveyId,
      questionId,
      evaluationId,
    );
    ...
    console.log(
      `✓ Wave 2 answers added: ${wave2Response.count || wave2Answers.length}`,
    );
    ...
    console.log(
      `  New jobs queued: ${continueResponse.summary.totalJobsQueued}`,
    );
    console.log(
      `  Is tracking survey: ${continueResponse.results[0].isTrackingSurvey}`,
    );
    ...
    console.log(`✓ Continue evaluation started:`);
    console.log(
      `  New jobs queued: ${continue2Response.summary.totalJobsQueued}`,
    );
    ...
    const finalResults = await client.getEvaluation(
      surveyId,
      questionId,
      evaluationId,
    );
    ...
    console.log(
      `  Expected: ${wave1Answers.length + wave2Answers.length + wave3Answers.length}`,
    );
    console.log();
    ...
    for (const evaluation of evaluationsList.evaluations) {
      console.log(
        `  - ${evaluation.name}: ${evaluation.processedAnswers}/${evaluation.totalAnswers} answers (${evaluation.is_completed ? "completed" : "in progress"})`,
      );
    }
    console.log();
    ...
    console.log(
      "  • Use createAnswers() to add new answers to existing questions",
    );
    ...
    console.log(
      "  • Track progress with listEvaluations() and getEvaluation()",
    );
    console.log();
...
function displayCodeFrameSummary(codeFrame: CodeFrameNode[]): void {
  ...
  for (const node of codeFrame || []) {
    ...
    console.log(
      `    • ${node.name}${childCount > 0 ? ` (${childCount} subcategories)` : ""}`,
    );
  }
}
